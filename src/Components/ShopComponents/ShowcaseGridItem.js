...
export class ShowcaseGridItem extends React.Component{
  ...
  render(){
    ...
    return(
      ...
      <button className={styles.buynow} onClick={() =>  this.navigationHook(`/Samsung/BuyNow?colors=${this.state.colors}&name=${this.state.header}&price=${this.state.price}&type=${this.state.deviceType}&images=${this.state.images}&spec1=${this.state.spec1}&spec2=${this.state.spec2}`)}> Buy Now </button>
      </div>
      )
  }
}